...
class FichaMedica():
    def __init__(self, row):
        ...
        self.operaciones = ' '.join(row[23].split()).capitalize()
        self.esquema_covid = ' '.join(row[24].split())

        # Medicamentos actuales
        self.medicamentos_actuales = ' '.join(row[25].split()).capitalize()
        self.medicamentos_sos = ' '.join(row[26].split()).capitalize()
        self.obs = ' '.join(row[27].split()).capitalize()
